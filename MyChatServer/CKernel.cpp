...
CKernel::CKernel() {
    pthread_rwlock_init(&m_sockRwlock, NULL);
    pthread_rwlock_init(&m_fileRwlock, NULL);
...
bool CKernel::startServer() {
    //2、连接mysql数据库
    if (!m_mysql->ConnectMySql("10.71.122.227", "wushuang", "wushuang", "mychat")) {
...
void CKernel::closeServer() {
    ...
    //m_lock.writeLock();
    pthread_rwlock_rdlock(&m_fileRwlock);
    ...
    pthread_rwlock_unlock(&m_fileRwlock);
    //m_lock.writeUnlock();

    pthread_rwlock_destroy(&m_sockRwlock);
    pthread_rwlock_destroy(&m_fileRwlock);
}
...
void CKernel::dealRegisterRq(char* buf, int nLen, long lSend) {
	...
	char sqlBuf[1024] = "";
    sprintf(sqlBuf, "select id from mychat.t_user where tel = '%s';", rq->tel);
	...
	if (lstSelect.size() > 0) {
        ...
        string temp = salt + rq->password;
        char password[1024]="";
        m_md5.funStrMD5SUM(temp.c_str(), temp.length(),password);
        //const char* password = m_md5.md5(().c_str();
		//cout << rq->tel <<"  " << rq->password << "	" << password << "	" << salt << endl;
		//将注册信息写入数据库
		memset(sqlBuf, 0, 1024);
        sprintf(sqlBuf, "insert into mychat.t_user (tel, password, salt) value('%s','%s','%s');", rq->tel, password, salt.c_str());
...
void CKernel::dealLoginRq(char* buf, int nLen, long lSend) {
	...
	list<string> lstSelect;
	char sqlBuf[1024] = "";
    sprintf(sqlBuf, "select id,password,salt from mychat.t_user where tel = '%s';", rq->tel);
	...
	if (lstSelect.empty()) {
        ...
        pthread_rwlock_rdlock(&m_sockRwlock);
        ...
        pthread_rwlock_unlock(&m_sockRwlock);
		...
		//计算相加的MD5的值
        char password[1024]="";
        m_md5.funStrMD5SUM(tempPassword.c_str(), tempPassword.length(),password);
        //const char* password = m_md5.md5(tempPassword.c_str()).c_str();
		...
		if (0 == strcmp(password, lstSelect.front().c_str())) {
            ...
            pthread_rwlock_wrlock(&m_sockRwlock);
...
void CKernel::dealChatRq(char* buf, int nLen, long lSend) {
	...
	list<string> lstSelect;
	char sqlBuf[1024] = "";
    sprintf(sqlBuf, "select idB from mychat.t_friend where idA = %d and idB = %d;", rq->userId, rq->friendId);
	if (!m_mysql->SelectMySql(sqlBuf, 1, lstSelect)) {
...
void CKernel::dealOfflineRq(char* buf, int nLen, long lSend) {
	...
	list<string> lstSelect;
	char sqlBuf[1024] = "";
    sprintf(sqlBuf, "select idB from mychat.t_friend where idA = %d;", rq->userId);
	if (!m_mysql->SelectMySql(sqlBuf, 1, lstSelect)) {
		cout << __func__ << "error" << endl;
		return;
	}
	...
	//断开与该用户的连接 关闭此线程
    close(m_mapIdToSocket[rq->userId]);
    pthread_rwlock_unlock(&m_sockRwlock);
    //TcpServer::m_threadLock.readUnlock();

	//TODO:将未发送完的文件记录到数据库中
    sprintf(sqlBuf, "select fileId from t_file where sendId = %d and t_flag = 0;", rq->userId);
	if (!m_mysql->SelectMySql(sqlBuf, 1, lstSelect)) {
		cout << __func__ << "error" << endl;
		return;
	}
	while (!lstSelect.empty()) {
		...
		if (m_FileIdToInfo.find(lstSelect.front()) != m_FileIdToInfo.end()) {
			...
			if (info) {
                sprintf(sqlBuf, "update t_file set pos = %lld where fileId = '%s';", info->filePos, info->fileId);
				if (!m_mysql->UpdateMySql(sqlBuf)) {
					cout << __func__ << "error" << endl;
                    pthread_rwlock_unlock(&m_fileRwlock);
                    //m_lock.readUnlock();
...
//	TcpServer::m_threadLock.UnLock();
	//通过用户id查询所有群聊
    sprintf(sqlBuf, "select groupId from mychat.t_groupmember where memberId = %d;", rq->userId);
	if (!m_mysql->SelectMySql(sqlBuf, 1, lstSelect)) {
		cout << __func__ << "error" << endl;
		return;
	}
	...
	while (!lstSelect.empty()) {
        ...
        sprintf(sqlBuf, "select memberId from mychat.t_groupmember where groupId = %d;", groupId);
...
void CKernel::dealAddFriendRq(char* buf, int nLen, long lSend) {
	...
	list<string> lstSelect;
	char sqlBuf[1024] = "";
    sprintf(sqlBuf, "select id, name from mychat.t_user where tel = %s;", rq->friendTel);
...
void CKernel::dealAddFriendRs(char* buf, int nLen, long lSend) {
	...
	list<string> lstSelect;
	char sqlBuf[1024] = "";
    sprintf(sqlBuf, "select name from mychat.t_user where id = %d;", rs->userId);
	if (!m_mysql->SelectMySql(sqlBuf, 1, lstSelect)) {
		cout << __func__ << "error" << endl;
		return;
	}
...
void CKernel::dealDeleteFriendRq(char* buf, int nLen, long lSend) {
	...
	//将好友关系表中的数据进行删除
    sprintf(sqlBuf, "delete from t_friend where idA = %d and idB = %d;", rq->userId, rq->friendId);
	if (!m_mysql->UpdateMySql(sqlBuf)) {
    ...
    sprintf(sqlBuf, "delete from t_friend where idA = %d and idB = %d;", rq->friendId, rq->userId);
	if (!m_mysql->UpdateMySql(sqlBuf)) {
		cout << "删除数据库失败，sql:" << sqlBuf << endl;
		rs.result = deletefriend_failed;
		m_pMediator->SendData((char*)&rs, sizeof(rs), lSend);
...
void CKernel::dealUserUpdataRq(char* buf, int nLen, long lSend) {
	...
	list<string> lstSelect;
	char sqlBuf[1024] = "";
    sprintf(sqlBuf, "select id from t_user where id = %d;", rq->userId);
	if (!m_mysql->SelectMySql(sqlBuf, 1, lstSelect)) {
		cout << __func__ << "error" << endl;
		return;
	}
	...
	//更新数据
    sprintf(sqlBuf, "update t_user set name = '%s',icon = %d, feeling = '%s' where id = %d;",
...
void CKernel::dealCreateGroupRq(char* buf, int nLen, long lSend) {
	...
	char sqlBuf[1024] = "";
    sprintf(sqlBuf, "insert into t_groupchat (name,userId) values ('%s',%d);", rq->groupName, rq->userId);
	if (!m_mysql->UpdateMySql(sqlBuf)) {
		cout << __func__ << "error" << endl;
	...
	list<string> lstSelect;
    sprintf(sqlBuf, "select max(id) from t_groupchat where userId = %d;", rq->userId);
	if (!m_mysql->SelectMySql(sqlBuf, 1, lstSelect)) {
		cout << __func__ << "error" << endl;
		return;
	}
...
void CKernel::dealJoinGroupRq(char* buf, int nLen, long lSend) {
	...
	list<string> lstSelect;
	char sqlBuf[1024] = "";
    sprintf(sqlBuf, "select id from t_groupchat where id = %d;", rq->groupId);
	if (!m_mysql->SelectMySql(sqlBuf, 1, lstSelect)) {
		cout << __func__ << "error" << endl;
		return;
	}
	...
	//插入群和该成员的关系表	使用存储过程
    sprintf(sqlBuf, "call joingroup(%d, %d);", rq->groupId, rq->userId);
	if (!m_mysql->UpdateMySql(sqlBuf)) {
		cout << __func__ << " error" << endl;
		return;
	}
	...
	if (!lstSelect.empty()) {
		...
		//发送群聊信息给所有在线的群成员
		GetGroupInfo(&info, groupId);
        sprintf(sqlBuf, "select memberId from t_groupmember where groupId = %d;", rq->groupId);
		if (!m_mysql->SelectMySql(sqlBuf, 1, lstSelect)) {
			cout << __func__ << "error" << endl;
			return;
		}
...
void CKernel::dealGroupUpDataInfoRq(char* buf, int nLen, long lSend) {
	...
	list<string> lstSelect;
	char sqlBuf[1024] = "";
    sprintf(sqlBuf, "select id from t_groupchat where id = %d and userId = %d;", info->groupId, info->userId);
	if (!m_mysql->SelectMySql(sqlBuf, 1, lstSelect)) {
		cout << __func__ << "error" << endl;
		return;
	}
	...
	//更新群聊信息
    sprintf(sqlBuf, "update t_groupchat set name = '%s', icon = %d where id = %d;", info->name, info->groupIconId, info->groupId);
	if (!m_mysql->UpdateMySql(sqlBuf)) {
		cout << __func__ << " error" << endl;
		return;
	}
    ...
    sprintf(sqlBuf, "select memberId from t_groupmember where groupId = %d;", info->groupId);
	if (!m_mysql->SelectMySql(sqlBuf, 1, lstSelect)) {
		cout << __func__ << "error" << endl;
		return;
	}
...
void CKernel::dealGroupDeleteRq(char* buf, int nLen, long lSend) {
	...
	list<string> lstSelect;
	char sqlBuf[1024] = "";
    sprintf(sqlBuf, "select id from t_groupchat where id = %d and userId = %d;", rq->groupId, rq->userId);
	if (!m_mysql->SelectMySql(sqlBuf, 1, lstSelect)) {
		cout << __func__ << "error" << endl;
		return;
	}
    ...
    sprintf(sqlBuf, "delete from t_groupchat where id = %d;", rq->groupId);
	if (!m_mysql->UpdateMySql(sqlBuf)) {
		cout << __func__ << "error" << endl;
		return;
	}
...
void CKernel::dealGroupExitRq(char* buf, int nLen, long lSend) {
	...
	char sqlBuf[1024] = "";
    sprintf(sqlBuf, "delete from t_groupmember where groupId = %d and memberId = %d;", rq->groupId, rq->userId);
	if (!m_mysql->UpdateMySql(sqlBuf)) {
		cout << __func__ << "error" << endl;
		return;
	}
...
void CKernel::dealGroupMsg(char* buf, int nLen, long lSend) {
	...
	list<string> lstSelect;
	char sqlBuf[1024] = "";
    sprintf(sqlBuf, "select memberId from t_groupmember where groupId = %d and memberId != %d;", rq->groupId, rq->userId);
	if (!m_mysql->SelectMySql(sqlBuf, 1, lstSelect)) {
		cout << __func__ << "error" << endl;
		return;
	}
...
void CKernel::dealFriendSendFileRq(char* buf, int nLen, long lSend) {
    ...
    sprintf(path, "/Server/%d", rq->friendId);

    if (mkdir(path,0664) != 0 && errno != EEXIST) {
        perror("mkdir error");
	...
	//创建该文件 
    sprintf(path, "/Server/%d/%s", rq->friendId, rq->fileName);
    int pFile = CreateLargeFile(path, rq->fileSize);

    if (pFile == -1) {
        perror("CreateLargeFile error");
    ...
    //m_lock.writeLock();
    pthread_rwlock_wrlock(&m_fileRwlock);
...
void CKernel::dealFileBlockRq(char* buf, int nLen, long lSend) {
    ...
    lseek64(info->pFile, pos, SEEK_SET);
    long long  writeNum = write(info->pFile,rq->block, rq->blockSize);
    //write(rq->block, sizeof(char), rq->blockSize, info->pFile);
...
void CKernel::dealFriendSendFileRs(char* buf, int nLen, long lSend) {
    ...
    lseek64(info->pFile, info->filePos, SEEK_SET);
    rq.blockSize = read(info->pFile, rq.block, DEF_FILE_BLOCK_SIZE);
    //read(rq.block, sizeof(char), DEF_FILE_BLOCK_SIZE, info->pFile);
    pthread_rwlock_unlock(&m_fileRwlock);
    //m_lock.readUnlock();
...
void CKernel::dealFileStatusRq(char* buf, int nLen, long lSend) {
	...
	if (rq->kind == filekind_send) {
        ...
        lseek64(info->pFile, fileRq.filePos, SEEK_SET);
        fileRq.blockSize = read(info->pFile, fileRq.block, DEF_FILE_BLOCK_SIZE);
...
void CKernel::GetAllFriendInfo(int userId) {
	...
	//通过用户id查询好友列表
  	list<string> lstSelect;
	char sqlBuf[1024] = "";
    sprintf(sqlBuf, "select idB from mychat.t_friend where idA = %d;", userId);
	if (!m_mysql->SelectMySql(sqlBuf, 1, lstSelect)) {
		cout << __func__ << "error" << endl;
		return;
	}
...
void CKernel::GetUserInfo(STRU_TCP_FRIEND_INFO* info, int userId) {
	...
	list<string> lstSelect;
	char sqlBuf[1024] = "";
    sprintf(sqlBuf, "select name, feeling, icon from mychat.t_user where id = %d;", userId);
	if (!m_mysql->SelectMySql(sqlBuf,3, lstSelect)) {
		cout << __func__ << "error" << endl;
		return;
	}
...
void CKernel::GetAllGroupInfo(int userId) {
	...
	list<string> lstSelect;
	char sqlBuf[1024] = "";
    sprintf(sqlBuf, "select groupId from mychat.t_groupmember where memberId = %d;", userId);
	if (!m_mysql->SelectMySql(sqlBuf, 1, lstSelect)) {
		cout << __func__ << "error" << endl;
		return;
	}
...
void CKernel::GetGroupInfo(STRU_TCP_GROUOP_INFO* info, int groupId) {
	list<string> lstSelect;
	char sqlBuf[1024] = "";
    sprintf(sqlBuf, "select name, peonum, icon, userId from mychat.t_groupchat where id = %d;", groupId);
...
void CKernel::GetAllGroupMemberInfo(int groupId, int userId) {
	...
	list<string> lstSelect;
	char sqlBuf[1024] = "";
    sprintf(sqlBuf, "select memberId from mychat.t_groupmember where groupid = %d and memberId = %d;", groupId, userId);
	if (!m_mysql->SelectMySql(sqlBuf, 1, lstSelect)) {
		cout << __func__ << "error" << endl;
		return;
	}
...
void CKernel::GetGroupMemberInfo(STRU_TCP_GROUOP_MEMBER_INFO* info, int userId) {
	...
	list<string> lstSelect;
	char sqlBuf[1024] = "";
    sprintf(sqlBuf, "select name, icon from mychat.t_user where id = %d;", userId);
	if (!m_mysql->SelectMySql(sqlBuf, 2, lstSelect)) {
		cout << __func__ << "error" << endl;
		return;
	}
...
//创建指定大小文件
int CKernel::CreateLargeFile(char* filePath, unsigned long long fileSize)
{
    int file;
	//判断该文件是否存在
    if(access(filePath, F_OK) == 0) {
        file = open(filePath, O_RDWR , 0664);
    }else {
        file = open(filePath, O_RDWR | O_CREAT, 0664);
...
void CKernel::GetAllFile(int userId) {
	...
	while ((!Selectlist.empty()) && Selectlist.size() % 6 == 0) {
		...
		//打开文件
        info->pFile = open(info->filePATH, O_RDONLY);
		//存入map
        //m_lock.writeLock();
        pthread_rwlock_wrlock(&m_fileRwlock);
