...
#define DEF_IP                  ("192.168.211.128")

#define DEF_RECV_BUF_SIZE       (4096)
...
struct STRU_FILE_INFO {
    STRU_FILE_INFO() : filePos(0), pFile(nullptr), status(sendfile_continue),kind(filekind_send) {
        memset(fileId, 0, DEF_FILE_ID_SIZE);
        memset(fileName, 0, DEF_FILE_NAME_SIZE);
